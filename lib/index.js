...
class Service extends AdapterService {
  ...
  _singleOptions (params) {
    return { options: params.mongodb || params.options };
  }
  ...
  _find (params = {}) {
    ...
    const q = this.Model.find(query, this._singleOptions(params));
  ...
  _get (id, params = {}) {
    ...
    return this.Model.findOne(query, this._singleOptions(params)).then(data => {
      if (!data) {
  ...
  _create (data, params = {}) {
    ...
    const promise = Array.isArray(data)
      ? this.Model.insertMany(data.map(setId), this._singleOptions(params))
      : this.Model.insertOne(setId(data), this._singleOptions(params));
  ...
  _remove (id, params = {}) {
    ...
    return this._findOrGet(id, findParams)
      .then(items => {
        return this.Model.deleteMany(query, this._singleOptions(params))
          .then(() => items)
          .then(select(params, this.id));
      }).catch(errorHandler);
  }
